...
function App(props) {
    ...
    const leaveCall = () => {
        ...
        socket.emit('closeSession',caller)
    }
    ...
    socket.on('closeSession',()=>{
        alert('close Session event is emitted ')
    })
    ...
    if (receiveCall && !callAccepted) { 
    //    var d = window.confirm('There is one need to call you')
    //    if(d)
    //     answerCall()
    }
    ...
    return (
        ...
        <div style={{
           width:'30%',
           display:expand===true?'block':'none'
        }}> 
