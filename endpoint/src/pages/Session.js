...
const ringtoneSound = new Howl({src: [ringtone], loop: true, preload: true})
...
export default() => {
    ...
    if (type == 0) {
        ...
        socket.current.emit('startCallWithTutor', {
    ...
    socket.current.on('letStart', (id) => {
        callPeer(id)
        // alert(id)
    })

    socket.current.on('myIDGet', (data) => {
        setHeader(data);
    })


    function callPeer(id) { // if (id !== '' && users[id] && id !== yourID) {
    ...
    if (stream) {
        UserVideo = (
            <video className="userVideo" playsInline muted
                ref={userVideo}
                autoPlay/>
        );
    }
    ...
    if (callAccepted && isfullscreen) {
        PartnerVideo = (
            <video className="partnerVideo cover" playsInline
                ref={partnerVideo}
                autoPlay/>
        );
    ...
    if (receivingCall && !callAccepted && !callRejected) {
        ...
        //                 <button name="accept" className="alertButtonPrimary"
        //                     onClick={
        //                         () => acceptCall()
        //                 }>Accept</button>
        //                 <button name="reject" className="alertButtonSecondary"
        //                     onClick={
        //                         () => rejectCall()
        //                 }>Reject</button>
        //             </div>
    ...
    let videoControl;
    if (videoMuted) {
        videoControl = <span className="iconContainer"
            onClick={
                () => toggleMuteVideo()
        }>
            <img src={camerastop}
    ...
    let screenShare = <span className="iconContainer"
        onClick={
            () => shareScreen()
    }>
        <img src={share}
    ...
    let hangUp = <span className="iconContainer"
        onClick={
            () => endCall()
    }>
        <img src={hangup}
    ...
    if (isfullscreen) {
        fullscreenButton = <span className="iconContainer"
            onClick={
                () => {
                    setFullscreen(false)
                }
